---
title: "Care shift tracker for Neighbourhoods"
title-slide-attributes: 
  data-background-color: "#f9bf07"
  data-background-image: 'https://raw.githubusercontent.com/The-Strategy-Unit/assets/main/logo_black.svg'
  data-background-size: 10%
  data-background-position: 98% 2%
subtitle: "NNHIP community of practice, 4th March 2026"
author: "Andy Hood, Analytics Manager"
format:
  revealjs:
    embed-resources: true
    slide-number: c/t
    preview-links: false
    width: 1600
    height: 900
editor: source
toc: true
toc-title: "Today's set-menu"
---

```{r setup}
#| echo: false

setwd("C:/Users/andrewhood/OneDrive - NHS/NHIP/left shift/git_repo/care_shift_tracker/")
library(targets)
library(quarto)
library(tidyverse)
library(gt)

```

## Introduction

- Me & team\
- The project in context:\
  - Supplier induced demand\
  - Neighbourhoods IC\
  - Complementary to e.g. TPMA explorer, Evidence reviews, Logic models etc...\
- Objectives for today's session:\
  - Awareness\
  - User feedback

## TL;DR

- The concept is difficult to describe and measure\
- The data is often poor\
- Difficult to derive insights i.e. causal impacts\
- We've given it a go - hopefully useful for:\
  - Prioritising outcomes and system pinch-points\
  - Identifying historic impact points\
  - Finding peer-learning opportunities\
  - Developing theories around system change\
- **System dynamics** and **simulation** possibly a better analytical play\
- **Infrastructure** will be critical to facilitating any shifts in care [OPINION]

## Care shift concept

:::: {.columns}
::: {.column width="30%"}
![](careshift_ai_image.png)
:::

::: {.column width = "70%"}
Shifting care is a dynamic and complex process. Everyone would have a different idea of what it means to them, their services or communities.

The ideas and product journey was seeded through the New Hospitals Programme, steered by Community and Frailty programmes, made tangible by the 10-year plan, resourced and couched within NNHIP.

Trying to measure the shift:

- Generic versus condition/sub-population focus?\
- Whole system versus components?\
- Commissioner versus Locality versus Delivery?
:::
::::

## Indicators and themes

::::{.columns}

::: {.column width = "70%"}
```{r indicators}

readxl::read_excel("C:/Users/andrewhood/OneDrive - NHS/NHIP/left shift/indicators_short_list.xlsx")|>
  mutate(Polarity = ifelse(Polarity == 1, "▲", "▼")) |>
  gt() |>
  tab_style(style = list(cell_text(color = "orange")), locations = cells_body(columns = Polarity, rows = Polarity == "▼") ) |>
  tab_style( style = list(cell_text(color = "darkblue")), locations = cells_body(columns = Polarity, rows = Polarity == "▲") ) |>
  opt_all_caps() |>
  opt_stylize(style = 6, color = 'gray')

```
:::

::: {.column width = "30%"}
<div style="font-size: 0.7em">
There were **many many many** indicators we explored for inclusion in the tracker. As well as trying to keep the number manageable, we excluded measures that had:\
<br>
- significant **overlap** with others;\
- weren't as **relevant to frailty**;\
- that suffered from **poor data quality**;\
- that didn't have sufficient **time series**;\
- very difficult to **replicate**;\
- too **small numbers**;\
- that couldn't be wrangled into our 3 **unit levels**;\
- that didn't have **national coverage** and so on...
</div>
:::

::::

## The tracker tool

<style>
.highlight-box {
  position: absolute;
  top: 520px;      /* adjust to match your image */
  left: 160px;     /* adjust to match your image */
  width: 510px;    /* size of the highlight */
  height: 350px;
  border: 4px solid red;
  box-sizing: border-box;
}
</style>

- Experimental
- R Shiny interface\
- Data through multiple lenses\
- Multiple levels of interest\
- Up-to-date (Dec '25, 2-month lag)
- Updates semi-automated

<div class="fragment"> <img src="prototype_image.png" style="width: 80%;"> </div>

<div class="fragment highlight-box"></div>

## Limitations & Caveats

- Partial coverage of the shift concept\
- Still a collection of silo'd views\
- Data quality and consistency away from acute sector\
- Some data gymnastics to align indicators at the 3 levels

## Walk-through

![](tracker_pic.png)
<br>
<a href="https://connect.strategyunitwm.nhs.uk/nhnip_care_shift_tracker_dev/" target="_blank" 
   style="display:inline-block; padding:8px 16px; background:#f9bf07; color:black; border-radius:4px; text-decoration:none;">
   Launch the tracker
</a>

## The what next?

- Developing tool interface and visuals further?\
  - Is the content right?\
  - Are the visuals useful?\
  - Is there sufficient explanation?\
  - What else would add value?\
- Utilising more system-wide data (social care, primary care, screening & diagnosis?)\
- Links to evidence of impact/improvement\
- Concepts applied to different pathways/population sub groups?\
- Feed into system dynamics model and simulations

## Thank you and questions

![](hands_up.gif)


